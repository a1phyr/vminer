...
trait BufRead {
    fn read_one_line(&mut self, buf: &mut String) -> VmResult<usize>;
}
...
impl BufRead for &[u8] {
    fn read_one_line(&mut self, buf: &mut String) -> VmResult<usize> {
        ...
        *self = rest;
        ...
        buf.push_str(core::str::from_utf8(line).map_err(crate::VmError::new)?);
        Ok(line.len())
    }
}

#[cfg(feature = "std")]
pub fn parse_symbol_file<R: io::BufRead>(r: R, syms: &mut ModuleSymbolsBuilder) -> VmResult<()> {
    parse_symbol_file_inner(r, syms)
}

pub fn read_from_bytes(bytes: &[u8], syms: &mut ModuleSymbolsBuilder) -> VmResult<()> {
    parse_symbol_file_inner(bytes, syms)
}

fn parse_symbol_file_inner<R: BufRead>(mut r: R, syms: &mut ModuleSymbolsBuilder) -> VmResult<()> {
